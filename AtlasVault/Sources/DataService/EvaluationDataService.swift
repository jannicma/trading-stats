...
public struct EvaluationDataService {
    ...
    public func saveEvaluations(_ evaluations: [Evaluation], strategy: UUID) async -> Bool {
        var evaluationDtos: [BacktestEvaluationDto] = evaluations.map { eval in
            BacktestEvaluationDto(
                strategyUuid: strategy, asset: eval.symbol!, timeframe: eval.timeframe!,
                parameters: eval.paramSet, trades: eval.trades, wins: eval.wins,
                losses: eval.losses, winRate: eval.winRate,
                averageRMultiples: eval.averageRMultiples, expectancy: eval.expectancy,
                avgRRR: eval.avgRRR, sharpe: eval.sharpe, sortino: eval.sortino,
                maxDrawdown: eval.maxDrawdown, calmarRatio: eval.calmarRatio,
                profitFactor: eval.profitFactor, ulcerIndex: eval.ulcerIndex,
                recoveryFactor: eval.recoveryFactor, equityVariance: eval.equityVariance,
                returnSpread50: eval.returnSpread50)
        }
            ...
            try await DatabaseManager.shared.write { db in
                ...
                for idx in evaluationDtos.indices {
                    do {
                        ...
                        print("error at index: \(idx): \(error)")
                    }
                }
                ...
                for (i, dtoEvaluation) in evaluationDtos.enumerated() {
                    ...
                    let points: [BacktestEquityDto] = originalEvaluation.equityCurve.map { point in
                        BacktestEquityDto(
                            tradeNumber: point.step, equity: point.equity, evaluationId: evalId)
                    }
