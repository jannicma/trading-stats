...
struct IndicatorEngine {
    ...
    public func updateLastIndicator(for candles: [Candle], existing: [Double], indicator: Indicator) -> [Double] {
        var newValue = 0.0
        switch indicator{
        case .sma(let period):
    ...
    public func computeIndicators(for candles: [Candle], requiredIndicators: [Indicator]) -> [Indicator: [Double]] {
    ...
    private func updateSimpleMovingAverage(on candles: [Candle], period: Int) -> Double {
        ...
        let sum = candles[lastIndex - period ... lastIndex].reduce(0) { $0 + $1.close }
    ...
    private func updateStochasticOscillator(on candles: [Candle], kLength: Int, existing: [Double]) -> Double {
        let lastIndex = candles.count - 1
